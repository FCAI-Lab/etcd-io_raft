...
Index alternativeMajorityCommittedIndex(MajorityConfig c, mapAckIndexer l) {
  ...
  for (int i = 0; i < lenC; i++) {
    for (int j = 0; j < lenC;
         j++) { // Iterate from current index to higher indexes
      mapAckIndexer_content idy =
          *(mapAckIndexer_content *)vector_get(&idxToVotes, j);
      Index idx = (*(mapAckIndexer_content *)vector_get(&idToIdx, i)).idx;
      if (idy.id > idx) {
        continue;
      }
  ...
  for (int i = 0; i < lenC; i++) {
    mapAckIndexer_content m =
        *(mapAckIndexer_content *)vector_get(&idxToVotes, i);
    uint64_t idx = m.id;
    uint64_t n = m.idx;
